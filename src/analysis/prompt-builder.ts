...
export class PromptBuilder {
  ...
  static extractJson(responseText: string): string | null {
   ...
   */
  static buildDependencyExtractionPrompt(data: DependencyExtractionPromptData): string {
    ...
    if (!components || components.length === 0) {
      ...
      // Stage 1 should receive exactly one component (selected in Stage 0)
      throw new ErodeError(
        `buildDependencyExtractionPrompt expects exactly 1 component, got ${components.length}`,
        ErrorCode.COMPONENT_NOT_FOUND,
        `Component selection error: Expected 1 component but got ${components.length}. Component selection should happen in Stage 0.`,
...
${comp.technology ? `Technology: ${comp.technology}` : ''}`;
   ...
   */
  static buildComponentSelectionPrompt(data: ComponentSelectionPromptData): string {
...
${idx + 1}. **${c.id}**
   ...
   ${c.description ? `- Description: ${c.description}` : ''}`
    ...
    // Format files list (just paths, no diffs)
  ...
  /**
   * Helper: Build allowed dependencies section with relationship types
   */
  private static buildAllowedDependenciesSection(architectural: {
    ...
    if (architectural.relationships && architectural.relationships.length > 0) {
      // Group relationships by target to show all connection types
  ...
  /**
   * Helper: Build dependents section
   */
  private static buildDependentsSection(dependents: { name: string; type: string }[]): string {
    return dependents.length > 0
      ? dependents.map((d) => `  - ${d.name} (${d.type})`).join('\n')
      : '  - None';
  }
  /**
   * Build dependency changes section for PR analysis.
   * @returns Markdown-formatted string with added/modified/removed dependency sections,
   *          or a "no changes" message if there are no dependency changes.
   */
  private static buildDependencyChangesSection(dependencies: DependencyExtractionResult): string {
   ...
   */
  static buildDriftAnalysisPrompt(data: DriftAnalysisPromptData): string {
    const { changeRequest, component, dependencies, architectural } = data;
    const allowedDeps = this.buildAllowedDependenciesSection(architectural);
    const dependents = this.buildDependentsSection(architectural.dependents);
    const dependencyChangesSection = this.buildDependencyChangesSection(dependencies);
    const commitsSection = changeRequest.commits
      .slice(0, 10)
      .map((c) => `  - ${c.sha.substring(0, 7)}: ${c.message.split('\n')[0]} (${c.author})`)
      .join('\n');
    const commitsNote =
      changeRequest.commits.length > 10 ? `\n  ... and ${changeRequest.commits.length - 10} more commits` : '';
   ...
   */
  static buildModelGenerationPrompt(analysisResult: DriftAnalysisResult): string {
