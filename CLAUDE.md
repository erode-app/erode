...
- **`PromptBuilder`** (`packages/core/src/analysis/prompt-builder.ts`): Assembles prompts from markdown templates (`packages/core/src/analysis/prompts/*.md`) using `PromptLoader`'s `{{variable}}` substitution.
...
Custom error hierarchy in `packages/core/src/errors.ts`: `ErodeError` (base) → `ConfigurationError`, `ApiError`. All errors carry an `ErrorCode` enum, a user-facing message, and context metadata. `ApiError` auto-detects rate limiting, timeouts, and acceleration limits from provider responses.
