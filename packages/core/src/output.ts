...
const mapViolation = (v: DriftViolation) => ({
  severity: v.severity,
...
export function formatAnalysisAsComment(
  ...
  lines.push('---');
  lines.push('*Automated by [erode](https://github.com/erode-app/erode)*');
...
export function formatErrorAsComment(error: unknown): string {
  ...
  if (error instanceof ApiError) {
    if (error.code === ErrorCode.PROVIDER_RATE_LIMITED) {
      lines.push(
    ...
    } else if (error.code === ErrorCode.PROVIDER_TIMEOUT) {
      lines.push(
