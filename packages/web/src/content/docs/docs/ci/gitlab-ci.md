...
erode:
  ...
  variables:
    ERODE_GITLAB_TOKEN: $GITLAB_API_TOKEN
...
erode:
  ...
  entrypoint: ['']
  script:
    - git clone --depth 1 "https://gitlab-ci-token:${ERODE_GITLAB_TOKEN}@gitlab.com/group/architecture-model.git" /tmp/model
    - >
...
Set `ERODE_GITLAB_BASE_URL` to point at your instance:

```yaml
variables:
  ERODE_GITLAB_BASE_URL: https://gitlab.example.com
```
