...
When `open-pr` is enabled, Erode runs Stage 4 (Model Update) to generate a deterministic patch from the Stage 3 structured analysis data, then creates a pull request against the model repository with the updated relationship declarations.
